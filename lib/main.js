...
export default {
    ...
    for (let i = 0 ; i < e.path.length ; i++) {
      if (e.path[i] && e.path[i].classList && e.path[i].classList.contains('line')) {
        if (e.path[i].querySelector('.syntax--tasklist.syntax--task')) {
          ...
          }, 50) // ensure a right text editor is activated
          break
        }
      }
    }
  ...
  activateHandler() {
    atom.workspace.getElement().addEventListener("mousedown", this.eventHandlerBinded, {capture:true, passive:true})
  },

  deactivateHandler() {
    atom.workspace.getElement().removeEventListener("mousedown", this.eventHandlerBinded, {capture:true, passive:true})
  },

  consumeStatusBar(statusBar) {
    this.tasklistStatus = new TasklistStatus()
    statusBar.addLeftTile({item:this.tasklistStatus, priority:1e9})
  },
}
